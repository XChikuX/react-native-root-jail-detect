...
- `src/specs/AppStoreReceiptProbe.nitro.ts` — **internal** iOS edge HybridObject Nitro spec (`getReceiptState(): Promise<string>`, resolving to `'app_store'` / `'sandbox'` / `'none'`). Promise-based because the iOS 16+ path is StoreKit 2 `AppTransaction.shared`, which is `async`; iOS 15 and AppTransaction failure paths fall back to the legacy receipt-existence check. Not exported from `src/` and not referenced by any JS-facing API; consumed only by `cpp/HybridRootJailDetect.cpp`. Backed by Swift on iOS (`{ ios: 'swift'; android: 'c++' }`) and a no-op C++ stub on Android (resolves `'none'`). The receipt fallback is existence-only and spoofable on jailbroken devices, so the value is purely informational.
...
- `cpp/IOSChecks.hpp` / `.cpp` — conservative iOS-only probes (jailbreak artifact paths, `_dyld` loaded-image scan, `sysctl` debugger state, simulator flag). Body compiled under `#if defined(__APPLE__)` so Android and host builds stay safe. Routes every signal through `SignalCatalog::lookupSignal()` so weights and severities never drift from the catalog. The pure matching logic (artifact-path table, dyld image matcher) lives in `cpp/IOSMatchers.hpp` and is host-tested in `cpp/tests/IOSMatcherTests.cpp`.
- `cpp/IOSMatchers.hpp` — pure, side-effect-free iOS matching (v0.13.0): the evidence-backed artifact-probe table (`IOS_ARTIFACT_PROBES` — every literal verified against public tool docs; unverified v0.12.0 literals removed) and `matchLoadedImage()` (case-insensitive hook/Frida tokens plus the `/var/jb/` and `.jbroot` provenance rules). Header-only so host tests need no build-file changes.
...
- `cpp/HybridAppStoreReceiptProbe.hpp` / `.cpp` — no-op C++ stub of the `AppStoreReceiptProbe` HybridObject (returns an already-resolved `Promise<std::string>` of `"none"`), used on Android and host builds. The real implementation is the Swift `HybridAppStoreReceiptProbe` edge class on iOS; `cpp/HybridRootJailDetect.cpp` reaches it via the registry and chains its Promise for `getInstallOrigin()`. Same explicit `HybridObject(TAG)` constructor rule applies.
...
- `ios/HybridAppStoreReceiptProbe.swift` — Swift edge implementation of the `AppStoreReceiptProbe` HybridObject. Implements `getReceiptState() throws -> Promise<String>`. On iOS 16+ it reads the StoreKit 2 `AppTransaction.shared` result and maps the `.verified` transaction's `environment` (`.production` → `'app_store'`, `.sandbox` → `'sandbox'`; `.unverified`, `.xcode`, and errors fall back). On iOS 15 and any AppTransaction failure it falls back to `Bundle.main.appStoreReceiptURL` existence, classified by last path component (`receipt` → `'app_store'`, `sandboxReceipt` → `'sandbox'` for TestFlight, missing/unknown → `'none'`). `appStoreReceiptURL` is deprecated since iOS 18, so it is retained only as the iOS 15/failure fallback; that fallback is existence-only and documented as spoofable (positive direction) on jailbroken devices. `init()` marked `override` like `HybridUrlSchemeProbe`.
...
`getInstallOrigin()` is an informational provenance getter outside the scored path: Android maps the PackageManager installer of record to `'google_play'`/`'other'`/`'unknown'`, and iOS uses StoreKit 2 `AppTransaction` verification on iOS 16+ (legacy `StoreKit/receipt` vs the TestFlight `StoreKit/sandboxReceipt` existence fallback on iOS 15) to resolve `'app_store'`/`'testflight'`/`'unknown'`. It never emits signals and cannot affect `compromised`; the scored Android installer-origin signal (`android.install.origin.other`) is a separate path that flows through normal `DetectionSignal` aggregation.
...
- Keep anonymous-map, module-property, mount-chain, and property-consistency logic pure and fixture-testable. Permission failures from module/package probes are unavailable data, never positive findings.
...
- debugger status
- detection reasons
- `getInstallOrigin()` on both platforms (Android installer record vs iOS StoreKit `AppTransaction`/receipt; `'unknown'` fallback on both)
...
Native heuristics are difficult to validate with unit tests alone. Prefer extracting deterministic parsing/matching logic into pure functions where feasible, then test those functions natively. The Android baseline follows this split: `cpp/ProcParsers.*` (pure `/proc`/`/sys` parsing, anonymous maps, module props, mount candidates, and property consistency), `cpp/Scoring.hpp` (pure aggregation), `cpp/SignalCatalog.*` (id → weight lookup), and `cpp/InstallOriginResolver.*` (install-origin provenance mapping) are deterministic and fixture-testable; `cpp/AndroidProbes.*` and `cpp/AndroidChecks.*` carry the platform I/O. Each host-test suite exposes a single `run*Tests()` entry point; `cpp/tests/ProcParsersTests.cpp` owns the only `main()`, and a second `main()` breaks the `native-test` link with duplicate symbols. Keep device-level verification through the example app for filesystem, process, debugger, socket, and runtime-instrumentation behavior.
...
- `android.install.origin.other` (low 10, reliability 0.35) is a hypothesis signal — the installer of record is explicit-setup spoofable and absent on ADB/system installs, so it must not become sole-basis. Its informational, never-scored complement is the `getInstallOrigin()` getter.
...
### iOS detection policy (`cpp/IOSMatchers.hpp`, `cpp/IOSChecks.cpp`) — v0.13.0 remediation
- Probe literals must be evidence-backed. Removed as unverified: `/private/preboot/{jb,dopamine,palera1n}` (real layout is `/private/preboot/<UUID>/jb` via `/var/jb`), `/var/jb/.installed_{dopamine,palera1n}`, and both TrollStore bundle paths (TrollStore installs into normal containers; hijacks `apple-magnifier://` since v1.3 to defeat scheme probes). Re-add only with a source citation.
- `ios.sideload.trollstore` (low 5, rel 0.35) and `ios.jailbreak.{dopamine,palera1n}` are **parked**: public-contract ids, no active probe. Rootless jailbreaks are detected via `/var/jb` (incl. dangling-symlink `lstat` companion).
- Classification is a pure table lookup per path (no substring matching, no order coupling); hybrid rootless+rootful devices emit BOTH classes (40 combined — deliberate, reviewed in ScoringTests).
- dyld scan: case-insensitive tokens (`ellekit` in any capitalization), `cydiasubstrate`/`cynject` legacy tokens, plus provenance rules `/var/jb/` and `/.jbroot` → same `ios.dyld.hook` id. At most ONE dyld signal per pass — intentional, don't "fix".
- **roothide ceiling**: randomized `.jbroot-<id>` bootstrap under `/var/containers/Bundle` is a documented permanent FN for path checks; only loaded roothide images are caught (provenance rule). Never claim roothide coverage.
- `LSApplicationQueriesSchemes` prerequisite: undeclared schemes always return `false` (silent). Example app plist fixed in v0.13.0; plugin `schemeCap` prop defaults 50, 25 for iOS 27+-linked apps (where `canOpenURL` is deprecated).
